...
#
# 2080Ti variant of the A40 scale job (A40 FCA pool pends ~1 week, 2026-07-27). The 28-GPU
# GTX2080TI FCA pool is its own QoS and 2:1 CPU:GPU: 16 GPUs = only 32 cores ~= 117 SU/hr.
# All 2080Ti nodes have 4 GPUs. Measures the same multi-node NCCL scaling SHAPE; 11 GB VRAM
# is ample for 512^2x256 fp32. --gres is PER NODE (4 x 4 nodes = 16).
#SBATCH --cpus-per-task=2
#SBATCH --gres=gpu:GTX2080TI:4
#SBATCH --time=01:00:00
#SBATCH --output=bench_p3_2080_%j.out
#SBATCH --error=bench_p3_2080_%j.err
#
# 28 GPUs in the FCA 2080Ti pool; this asks for 16 of them across 4 nodes.

# T8 multi-node scaling: 4/8/16 GTX2080TI at fp32, the NASA-cluster proxy for NCCL-over-IB. This is
...
# NCCL: PCIe GPU P2P is BROKEN on Savio GPU nodes (measured on savio4_gpu, known issue per
# Savio staff cluster-wide; bench/nccl_repro.py passes only with P2P off -> SHM transport).
...
export RMHD_PRECISION=32   # A40 fp64 is ~1/32 of fp32; the fp64 anchor is the V100 job
...
# --gres=gpu:GTX2080TI:2 (the lower-pend-risk 16-GPU shape) needs NO edits below this line
GPUS_PER_NODE=${GPUS_PER_NODE:-$(( SLURM_NTASKS / SLURM_NNODES ))}

NX=512; NZ=256          # nz_local = 16 at 16 GPUs; 512^2x256 fp32 ~ 0.6 GB/state fits 11 GB cards
STEPS="nb20 nr4"
...
run() { local n=$1; shift; local gpn=$(( n < GPUS_PER_NODE ? n : GPUS_PER_NODE )); \
        ...
        --cpus-per-task=2 --gres=gpu:GTX2080TI:"$gpn" $bind \
...
# One profiled case (F2(b) stream syncs at scale); reading it: SAVIO_GPU_SETUP.md "Reading the profile".
