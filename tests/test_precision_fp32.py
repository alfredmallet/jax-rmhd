...
def test_default_precision_is_32():
    ...
    # jax_enable_x64 -- the honest check of the production default.
    ...
    with checks() as c:
        ...
        c.check("default precision is 32 (jax_enable_x64 False)",
                "X64_IS False" in out.stdout, out.stdout[-200:])
...
def test_state_dtypes_are_fp32():
    ...
    with checks() as c:
        c.check("this session runs at fp32", not jax.config.jax_enable_x64)
        ...
        c.check("t is a 32-bit scalar",
                jnp.asarray(state.t).dtype == jnp.float32)
